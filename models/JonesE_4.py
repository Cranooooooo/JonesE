...
class JonesE_4(Model):
    def __init__(self, config):
        super(JonesE_4, self).__init__(config)
    ...
    def init_weights(self):
        if False:
            ...
            # Jones Matrix 4
            Eta_4, Thet_4 = self.quaternion_init(self.config.entTotal, self.config.hidden_size, type='relation')
            Eta_4, Thet_4 = torch.from_numpy(Eta_4), torch.from_numpy(Thet_4)
            self.rel_Eta_4.weight.data = Eta_4.type_as(self.rel_Eta_4.weight.data)
            self.rel_Thet_4.weight.data = Thet_4.type_as(self.rel_Thet_4.weight.data)
    ...
    def _calc(self, E_x_h, E_y_h, Phi_x_h, Phi_y_h, E_x_t, E_y_t, Phi_x_t, Phi_y_t,
        ...
        jonesMat4 = self._jonesMat(Eta_4, Thet_4)
        ...
        jonesVec_ho = [jonesMat4[0] * jonesVec_ho[0] + jonesMat4[1] * jonesVec_ho[1],
                       jonesMat4[2] * jonesVec_ho[0] + jonesMat4[3] * jonesVec_ho[1]]
    ...
    def forward(self):
        ...
        regul2 = (torch.mean(torch.relu(torch.abs(Eta_1)-torch.pi) ** 2)
                 ...
                 + torch.mean(torch.relu(torch.abs(Eta_4) - torch.pi) ** 2)
                 + torch.mean(torch.relu(torch.abs(Thet_4) - torch.pi) ** 2)
                  )
