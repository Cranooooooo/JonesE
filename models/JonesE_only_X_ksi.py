...
class JonesE_only_X(Model):
    def __init__(self, config):
        ...
        self.emb_ksi = nn.Embedding(self.config.entTotal, self.config.hidden_size, max_norm=torch.pi/2)
    ...
    def init_weights(self):
        nn.init.xavier_uniform_(self.emb_ksi.weight.data)
    ...
    def prepare_enti_rela(self):
        E_h_ksi = torch.relu(self.emb_ksi(self.batch_h))
        E_x_h = torch.cos(E_h_ksi)
        E_y_h = torch.sin(E_h_ksi)
        Phi_x_h = torch.relu(self.emb_Phi_x(self.batch_h))-torch.pi
        Phi_y_h = torch.relu(self.emb_Phi_y(self.batch_h))-torch.pi

        E_t_ksi = torch.relu(self.emb_ksi(self.batch_t))
        E_x_t = torch.cos(E_t_ksi)
        E_y_t = torch.sin(E_t_ksi)
