    ...
    def _interfer(self, h_wf_list, t_wf_list):
        ...
        for wf_idx in range(len(h_wf_list)):
            ...
            if self.use_interfer:  # Use Interference
                ...
                head_amp_norm = F.normalize(torch.cat([h_wf[0].unsqueeze(-1), h_wf[1].unsqueeze(-1)], dim=2), p=2, dim=2)
                tail_amp_norm = F.normalize(torch.cat([t_wf[0].unsqueeze(-1), t_wf[1].unsqueeze(-1)], dim=2), p=2, dim=2)
                amp_in_prod = torch.sum(head_amp_norm* tail_amp_norm, dim=2)

                head_phase_norm = F.normalize(torch.cat([h_wf[2].unsqueeze(-1), h_wf[3].unsqueeze(-1)], dim=2), p=2, dim=2)
                tail_phase_norm = F.normalize(torch.cat([t_wf[2].unsqueeze(-1), t_wf[3].unsqueeze(-1)], dim=2), p=2, dim=2)
