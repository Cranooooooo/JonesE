...
class JonesE_Inter_Dual(Model):
    ...
    def _interfer(self, h_wf_list, t_wf_list):
        ...
        for wf_idx in range(len(h_wf_list)):
            ...
            if self.use_interfer:  # Use Interference
                ...
                head_amp_norm = torch.cat([h_wf[0].unsqueeze(-1), h_wf[1].unsqueeze(-1)], dim=2)
                tail_amp_norm = torch.cat([t_wf[0].unsqueeze(-1), t_wf[1].unsqueeze(-1)], dim=2)
                # amp_in_prod = torch.sum(head_amp_norm * tail_amp_norm, dim=2).squeeze(-1)
                amp_in_prod = torch.sum(torch.abs(head_amp_norm - tail_amp_norm), dim=2).squeeze(-1)

                head_phase_norm = torch.cat([h_wf[2].unsqueeze(-1), h_wf[3].unsqueeze(-1)], dim=2)
                tail_phase_norm = torch.cat([t_wf[2].unsqueeze(-1), t_wf[3].unsqueeze(-1)], dim=2)
                phase_in_prod = torch.sum(torch.abs(head_phase_norm - tail_phase_norm), dim=2).squeeze(-1)
                # phase_in_prod = torch.sum(head_phase_norm * tail_phase_norm, dim=2).squeeze(-1)

                total_interfer_intense += 2./(1.+amp_in_prod+phase_in_prod)
    ...
    def _calc(self, E_x_h, E_y_h, Phi_x_h, Phi_y_h, E_x_t, E_y_t, Phi_x_t, Phi_y_t,
        ...
        t_wf_out_list = self._jonesMat_trans(t_wf_list, J_mat_2)

        # Optical Interference calculation
        score_r = self._interfer(h_wf_out_list, t_wf_out_list)
        score_r = score_r-1. # Since Max score is 1, Min score is 0
    ...
    def loss(self, score, regul, regul2):
        score_loss = torch.mean(self.criterion(10* score * self.batch_y))
