...
class JonesE(Model):
    def __init__(self, config):
        super(JonesE, self).__init__(config)
    ...
    def prepare_enti_rela(self):
        ...
        return E_x_h, E_y_h, Phi_x_h, Phi_y_h, E_x_t, E_y_t, Phi_x_t, Phi_y_t, Eta_1, Thet_1, Eta_2, Thet_2
    ...
    def forward(self):
        ...
        regul2 = (torch.mean(torch.relu(torch.abs(Eta_1)-torch.pi) ** 2)
                 ...
                 + torch.mean(torch.relu(torch.abs(Eta_1)-torch.pi) ** 2)
                 + torch.mean(torch.relu(torch.abs(Thet_1)-torch.pi) ** 2))
