...
class Orin_JonesE(Model):
    def __init__(self, config):
        super(Orin_JonesE, self).__init__(config)
        ...
        self.rel_Phi_1 = nn.Embedding(self.config.relTotal, self.config.hidden_size)
        ...
        self.rel_Phi_2 = nn.Embedding(self.config.relTotal, self.config.hidden_size)
    ...
    def init_weights(self):
        if False:
            ...
            # Jones Matrix 1
            Eta_1, Thet_1, Phi_1 = self.quaternion_init(self.config.entTotal, self.config.hidden_size, type='relation')
            Eta_1, Thet_1, Phi_1 = torch.from_numpy(Eta_1), torch.from_numpy(Thet_1), torch.from_numpy(Phi_1)
            ...
            self.rel_Phi_1.weight.data = Phi_1.type_as(self.rel_Phi_1.weight.data)

            # Jones Matrix 2
            Eta_2, Thet_2, Phi_2 = self.quaternion_init(self.config.entTotal, self.config.hidden_size, type='relation')
            Eta_2, Thet_2, Phi_2 = torch.from_numpy(Eta_2), torch.from_numpy(Thet_2), torch.from_numpy(Phi_2)
            ...
            self.rel_Phi_2.weight.data = Phi_2.type_as(self.rel_Phi_2.weight.data)
    ...
    # Build Jone Matrix
    def _jonesMat(self, Eta, Thet, Phi):
        ...
        jonesMat12 = (1. - cos_Eta) * cos_Phi * cos_Thet * sin_Thet
        jonesMat21 = (1. - cos_Eta) * cos_Phi * cos_Thet * sin_Thet
    ...
    def _calc(self, E_x_h, E_y_h, Phi_x_h, Phi_y_h, E_x_t, E_y_t, Phi_x_t, Phi_y_t,
              Eta_1, Thet_1, Phi_1, Eta_2, Thet_2, Phi_2):
        # Build Jones Vector
        jonesVec_h = self._jonesVec(E_x_h, E_y_h, Phi_x_h, Phi_y_h)
        jonesVec_t = self._jonesVec(E_x_t, E_y_t, Phi_x_t, Phi_y_t)

        # Build Jones Matrix
        jonesMat1 = self._jonesMat(Eta_1, Thet_1, Phi_1)
        jonesMat2 = self._jonesMat(Eta_2, Thet_2, Phi_2)
        ...
        score_r = jonesVec_ho[0] * jonesVec_t[0] + jonesVec_ho[1] * jonesVec_t[1]

        return torch.sum(score_r, -1)

    def loss(self, score, regul, regul2):
        return (torch.mean(self.criterion(score * self.batch_y))
                + self.config.lmbda * regul
                + self.config.lmbda * regul2)

    def forward(self):
        E_x_h = self.emb_E_x(self.batch_h)
        E_y_h = self.emb_E_y(self.batch_h)
        Phi_x_h = self.emb_Phi_x(self.batch_h)
        Phi_y_h = self.emb_Phi_y(self.batch_h)

        E_x_t = self.emb_E_x(self.batch_t)
        E_y_t = self.emb_E_y(self.batch_t)
        ...
        Phi_1 = self.rel_Phi_1(self.batch_r)
        Eta_2 = self.rel_Eta_1(self.batch_r)
        Thet_2 = self.rel_Thet_1(self.batch_r)
        Phi_2 = self.rel_Phi_1(self.batch_r)
        ...
        regul = (torch.mean(torch.abs(E_x_h) ** 2)
                 ...
                 + torch.mean(torch.abs(Phi_x_h) ** 2)
                 + torch.mean(torch.abs(Phi_y_h) ** 2)
                 + torch.mean(torch.abs(E_x_t) ** 2)
                 + torch.mean(torch.abs(E_y_t) ** 2)
                 + torch.mean(torch.abs(Phi_x_t) ** 2)
                 + torch.mean(torch.abs(Phi_y_t) ** 2)
                 )
        regul2 = (torch.mean(torch.abs(Eta_1) ** 2)
                  + torch.mean(torch.abs(Thet_1) ** 2)
                  + torch.mean(torch.abs(Phi_1) ** 2)
                  + torch.mean(torch.abs(Eta_2) ** 2)
                  + torch.mean(torch.abs(Thet_2) ** 2)
                  + torch.mean(torch.abs(Phi_2) ** 2))
