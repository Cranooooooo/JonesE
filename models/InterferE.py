...
class InterferE(Model):
    def __init__(self, config):
        super(InterferE, self).__init__(config)
        self.o_x_alpha = 0
        ...
        if self.o_x_alpha != 0:
            self.rel_obj_x_o = nn.Embedding(self.config.relTotal, self.config.hidden_size)
            self.rel_sbj_x_o = nn.Embedding(self.config.relTotal, self.config.hidden_size)
        self.rel_obj_x_delta = nn.Embedding(self.config.relTotal, self.config.hidden_size)
        self.rel_sbj_x_delta = nn.Embedding(self.config.relTotal, self.config.hidden_size)
        self.rel_obj_y_delta = nn.Embedding(self.config.relTotal, self.config.hidden_size)
        self.rel_sbj_y_delta = nn.Embedding(self.config.relTotal, self.config.hidden_size)
    ...
    def init_weights(self):
        ...
        nn.init.xavier_uniform_(self.rel_obj_x_delta.weight.data)
        nn.init.xavier_uniform_(self.rel_sbj_x_delta.weight.data)
        nn.init.xavier_uniform_(self.rel_obj_y_delta.weight.data)
        nn.init.xavier_uniform_(self.rel_sbj_y_delta.weight.data)
        nn.init.xavier_uniform_(self.rel_w.weight.data)
        if self.o_x_alpha != 0:
            nn.init.xavier_uniform_(self.rel_obj_x_o.weight.data)
            nn.init.xavier_uniform_(self.rel_sbj_x_o.weight.data)


    #Calculate the inner product
    def _calc(self, E_x_h, E_y_h, Phi_x_h, Phi_y_h, E_x_t, E_y_t, Phi_x_t, Phi_y_t,
                    rel_obj_x_delta, rel_sbj_x_delta, rel_obj_y_delta, rel_sbj_y_delta,
                    rel_obj_x_o=0, rel_sbj_x_o=0):

        # Obj wave after retarder along o-axis and e-axis
        norm_x_h = E_x_h / (E_x_h ** 2 + E_y_h ** 2) ** (1 / 2)
        norm_y_h = E_y_h / (E_x_h ** 2 + E_y_h ** 2) ** (1 / 2)
        norm_x_t = E_x_t / (E_x_t ** 2 + E_y_t ** 2) ** (1 / 2)
        norm_y_t = E_y_t / (E_x_t ** 2 + E_y_t ** 2) ** (1 / 2)

        score_r = (torch.cos(Phi_x_h+rel_obj_x_delta-Phi_x_t-rel_sbj_x_delta)*E_x_h*E_x_t+\
                   torch.cos(Phi_y_h+rel_obj_y_delta-Phi_y_t-rel_sbj_y_delta)*E_y_h*E_y_t)* \
                  (norm_x_h * norm_x_t + norm_y_h * norm_y_t)/(E_x_h*E_x_t+E_y_h*E_y_t)
    ...
    def prepare_enti_rela(self):
        E_x_h = torch.sigmoid(self.emb_E_x(self.batch_h))
        E_y_h = torch.sigmoid(self.emb_E_y(self.batch_h))
        ...
        E_x_t = torch.sigmoid(self.emb_E_x(self.batch_t))
        E_y_t = torch.sigmoid(self.emb_E_y(self.batch_t))
        ...
        rel_obj_x_delta = self.rel_obj_x_delta(self.batch_r)
        rel_sbj_x_delta = self.rel_sbj_x_delta(self.batch_r)
        rel_obj_y_delta = self.rel_obj_y_delta(self.batch_r)
        rel_sbj_y_delta = self.rel_sbj_y_delta(self.batch_r)
        # rel_obj_x_delta = torch.tanh(self.rel_obj_x_delta(self.batch_r))*torch.pi
        # rel_sbj_x_delta = torch.tanh(self.rel_sbj_x_delta(self.batch_r))*torch.pi
        # rel_obj_y_delta = torch.tanh(self.rel_obj_y_delta(self.batch_r))*torch.pi
        # rel_sbj_y_delta = torch.tanh(self.rel_sbj_y_delta(self.batch_r))*torch.pi
    ...
    def forward(self):
        ...
        rel_obj_x_delta, rel_sbj_x_delta, rel_obj_y_delta, rel_sbj_y_delta = self.prepare_enti_rela()
        ...
        regul2 = (torch.mean(torch.relu(torch.abs(rel_obj_x_delta)-torch.pi) ** 2)
                 + torch.mean(torch.relu(torch.abs(rel_sbj_x_delta)-torch.pi) ** 2)
                 + torch.mean(torch.relu(torch.abs(rel_obj_y_delta)-torch.pi) ** 2)
                 + torch.mean(torch.relu(torch.abs(rel_sbj_y_delta)-torch.pi) ** 2))
