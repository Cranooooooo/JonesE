...
class Config(object):
    ...
    def set_train_model(self, model):
        ...
        if self.start_from != None:
            self.trainModel.load_state_dict(torch.load(self.start_from))
    ...
    def valid(self, model):
        # Load text info
        # with open(self.in_path+'valid_text_cl.json', 'r') as f:
        ...
        for i in range(self.validTotal):
            ...
            #sys.stdout.flush()
            # Test Head entity, valid_h: [0, 1, 2, ..., 40942]
            # Tail keep the same
            ...
            # # CL text here
            # gt_tail = self.valid_t[0]
            # gt_rela = self.valid_r[0]
            # top_10_h = res.argsort()


            # Test Tail entity, valid_t: [0, 1, 2, ..., 40942]
            # Head keep the same
        ...
        #     # CL text here
        #     gt_head = self.valid_h[0]
        #     top_10_t = res.argsort()
        #
        #     # Print GT text
        #     gt_res = valid_text_dict[str([gt_head, gt_tail, gt_rela])]
        #     if gt_tail not in top_10_t[0:10].tolist() or gt_head not in top_10_h[0:10].tolist():
        #         bad_count+=1
        #         bad_res_record[str([gt_head, gt_tail, gt_rela])] = [top_10_h.tolist().index(gt_head),
        #                                                             top_10_t.tolist().index(gt_tail),
        #                                                             gt_res]
        #         print('GT Head rank to: ', top_10_h.tolist().index(gt_head))
        #         print('GT Tail rank to: ', top_10_t.tolist().index(gt_tail))
        #         print(f'Ground Truth: {[gt_head, gt_tail, gt_rela]} / {gt_res}\n')
        # print(f'Bad Count : {bad_count}')
        # with open(f'./bad_res_analysis/{self.model_name}_{self.dataset_name}_Valid.json', 'w') as f:
        #     json.dump(bad_res_record, f)
    ...
    def train(self):
        ...
        best_model = None
        bad_counts = 0
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor=.64, mode='max', verbose=True)
        for epoch in range(self.train_times):
            ...
            if (epoch+1) %self.valid_steps == 0: scheduler.step(hit10)
